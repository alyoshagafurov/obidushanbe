...
import { getCashiers, createStaff, setStaffActive } from '../../api/admin';
import { apiErrorMessage } from '../../lib/api';
import { colors, spacing } from '../../theme';
import { t } from '../../i18n';

export function ManageCashiersScreen() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['admin-cashiers'] });

  const addMutation = useMutation({
    mutationFn: () =>
      createStaff({ phone: form.phone.replace(/\s/g, ''), name: form.name.trim(), role: 'CASHIER' }),
    onSuccess: () => {
      invalidate();
      setModalOpen(false);
  ...
  return (
    <Screen padded={false}>
      <FlatList
        ...
        ListHeaderComponent={
          <Button title={t().admin.addCashier} onPress={() => setModalOpen(true)} style={{ marginBottom: spacing.md }} />
        }
        renderItem={({ item }) => (
      ...
      <Modal visible={modalOpen} animationType="slide" transparent onRequestClose={() => setModalOpen(false)}>
        <View style={styles.modalBg}>
          <View style={styles.modal}>
            <H2 style={{ fontSize: 18, marginBottom: spacing.md }}>{t().admin.addCashier}</H2>
